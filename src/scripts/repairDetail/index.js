...
import {handleSelectData} from '../store/types/counter'
import {connect} from 'wepy-redux'

@connect({
  tempData(state) {
    return state.counter.equipInfo
  }
...
export default class Index extends wepy.page {
  ...
  data = {
    ...
    alertData: {
      ...
      //
      endBoard: {
        array: ['V2主板', 'V3主板', 'V5主板', 'V7主板', 'V8主板', '大绿壳主板', '有线主板', '卡尔主板', '免刷卡'],
        endBoardIndex: 0
    ...
    for (let key in data) {
      switch (key) {
        ...
        case 'hardwareType':
          //  硬件类型
          newData = []
          data.hardwareType.filter(function (item) {
            item.selected === true ? newData.push(item.customName || item.name) : ''
          })
          selectOptions.hardwareType = newData.join(',')
          selectOptions.hardwareTypeSelected = newData.join(',').length > 14 ? newData.join(',').substring(0, 14) + '...' : newData.join(',')
          break
        case 'faultType':
          newData = []
        ...
        case 'treatmentMeasure':
          newData = []
          data.treatmentMeasure.filter(function (item) {
            item.selected === true ? newData.push(item.customName || item.name) : ''
          })
          selectOptions.treatmentMeasure = newData.join(',')
          selectOptions.treatmentMeasureSelected = newData.join(',').length > 14 ? newData.join(',').substring(0, 14) + '...' : newData.join(',')
          break
        case 'faultPhenomenon':
          newData = []
          data.faultPhenomenon.filter(function (item) {
            item.selected === true ? newData.push(item.customName || item.name) : ''
          })
          selectOptions.faultPhenomenon = newData.join(',')
          selectOptions.faultPhenomenonSelected = newData.join(',').length > 14 ? newData.join(',').substring(0, 14) + '...' : newData.join(',')
          break
        case 'faultCause':
          newData = []
          data.faultCause.filter(function (item) {
            item.selected === true ? newData.push(item.customName || item.name) : ''
          })
          selectOptions.faultCause = newData.join(',')
          selectOptions.faultCauseSelected = newData.join(',').length > 14 ? newData.join(',').substring(0, 14) + '...' : newData.join(',')
          break
      }
    }
    this.handleCustom()
  ...
  async updateEquipInfo() {
    ...
    this.alertData.submitData.faultMainboard = this.alertData.submitData.faultMainboard || this.repairData.faultMainboard
    this.alertData.submitData.terminalState = this.alertData.submitData.terminalState || this.repairData.terminalState
  ...
  handleDefaultSelect(type) {
    ...
    switch (type) {
      ...
      case 'hardwareType':
        ...
        console.log('222')
        this._handleSelectData(nameString, 'hardwareType')
        break
      case 'faultPhenomenon':
        // 故障现象
        nameString = this.repairData.faultPhenomenon
        this._handleSelectData(nameString, 'faultPhenomenon')
        break
      case 'faultCause':
        // 故障原因
        nameString = this.repairData.faultCause
        this._handleSelectData(nameString, 'faultCause')
        break
      case 'treatmentMeasure':
        // 处理措施
        nameString = this.repairData.treatmentMeasure
        this._handleSelectData(nameString, 'treatmentMeasure')
        break
      case 'remark':
        // 备注
  ...
  _handleSelectData(nameString, selectName) {
    ...
    if (data[`${selectName}`][0].name === '自定义') {
      if (typeof name === 'object') {
        name.forEach(function (item) {
          ...
          if (flag === false) {
            if (item.slice(4).length) {
              data[`${selectName}`][0].customName = item.slice(4)
              data[`${selectName}`][0].selected = true
            } else {
              data[`${selectName}`][0].selected = false
            }
          }
        })
      }
    ...
    this.echoSelectedData();
    console.log('测试测试');
    this.methods.handleSelectData(data);
  ...
  handleCustom() {
    ...
    let data = this.$parent.globalData;
    ['remark', 'hardwareType', 'treatmentMeasure', 'faultPhenomenon', 'faultCause'].forEach((item) => {
      if (this.$parent.globalData[`${item}`][0].selected === true && this.$parent.globalData[`${item}`][0].customName == undefined) {
        this.selectOptions[`${item}Selected`] = this.selectOptions[`${item}Selected`].length > 4 && this.selectOptions[`${item}Selected`].slice(0, 3) == '自定义' ? this.selectOptions[`${item}Selected`].slice(4) : this.selectOptions[`${item}Selected`].slice(0)
      }
    })
    this.$apply()
  }
  ...
  selectDiyInfo() {
    ...
    // 提交时，将自定义项放在最后边,同时将最后一项改为 自定义：'xxx'
      ...
      ['remark', 'hardwareType', 'treatmentMeasure', 'faultPhenomenon', 'faultCause'].forEach(item => {
        if (key === item) {
          if (this.$parent.globalData[key][0].selected == true) {
            switch (key) {
              case 'remark':
                if (this.selectOptions.remark.split(',')[0] == '自定义') {
                  tempStr = '自定义:'
                } else {
                  tempStr = '自定义:' + this.selectOptions.remark.split(',')[0]
                }
                ...
                break
              case 'hardwareType':
                if (this.selectOptions.hardwareType.split(',')[0] == '自定义') {
                  tempStr = '自定义:'
                } else {
                  tempStr = '自定义:' + this.selectOptions.hardwareType.split(',')[0]
                }
                ...
                break
              case 'treatmentMeasure':
                if (this.selectOptions.treatmentMeasure.split(',')[0] == '自定义') {
                  tempStr = '自定义:'
                } else {
                  tempStr = '自定义:' + this.selectOptions.treatmentMeasure.split(',')[0]
                }
                ...
                break
              case 'faultPhenomenon':
                if (this.selectOptions.faultPhenomenon.split(',')[0] == '自定义') {
                  tempStr = '自定义:'
                } else {
                  tempStr = '自定义:' + this.selectOptions.faultPhenomenon.split(',')[0]
                }
                ...
                break
              case 'faultCause':
                if (this.selectOptions.faultCause.split(',')[0] == '自定义') {
                  tempStr = '自定义:'
                } else {
                  tempStr = '自定义:' + this.selectOptions.faultCause.split(',')[0]
                }
